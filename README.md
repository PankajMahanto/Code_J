...
**Novelties**
- **SGP-E** — *Spectral Graph-Infused Hierarchical Poincaré Encoder*: semantic graph convolution + hyperbolic Poincaré latent manifold + Fisher–Rao information-geometry KL, fused with sentence-transformer contextual embeddings (replaces static GloVe).
...
- **SCAD** — *Sinkhorn Concept-Anchor Decoder*: learnable concept anchors + low-rank Mahalanobis cost + entropic optimal transport (Sinkhorn) + orthogonal regularisation on the topic-word matrix.

**Stability fixes (v4)**
- KL annealing β: 0 → 1 over the first 20 epochs (prevents posterior collapse).
- Gradient clipping with `max_norm=1.0` on every step.
- Orthogonal regularisation on the decoder topic-word matrix (`||β β^T − I||²`) to drive Inter-cosine ≤ 0.30.
- Aggressive vocabulary filtering (`min_word_count`, `max_word_freq`, plus a hapax sweep).
...
│   └── utils/
│       ├── __init__.py
│       ├── contextual_embeddings.py   # sentence-transformer doc / vocab encoder
│       ├── pmi.py
│       ├── config.py
│       └── logging_utils.py
├── scripts/
