"""
============================================================================
 src/training/ablation.py  [v4 — contextual + orthogonal regularisation]
 ---------------------------------------------------------------------------
...
class _AblationWrapper(nn.Module):
    def __init__(self, encoder: nn.Module, decoder: nn.Module, has_ctx: bool):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor,
        ...
        if self.has_ctx:
            z_h, theta, mu, logvar, kl = self.encoder(
                x, ctx=ctx, temperature=temperature
            )
        else:
...
def _build_variant(variant: str, cfg, V: int,
    ...
    return _AblationWrapper(enc, dec, has_ctx=has_ctx).to(device)
...
def _train_variant(variant: str, cfg, V: int, loader,
    ...
    orth_w    = float(getattr(cfg.loss_weights, "orth", 1.0))
    ...
    results = {
        ...
        "C_UCI":     round(c_uci(topics_gated, stats,
                                 cfg.evaluation.top_n_words), 4),
        "Diversity": round(topic_diversity(topics_gated, cfg.evaluation.top_n_words), 4),
        "Intra":     round(intra_topic_cosine(topics_gated, emb_dict,
                                              cfg.evaluation.top_n_words), 4),
...
def run_ablation_suite(cfg, variants: List[str] = None,
    ...
    ctx_model = getattr(cfg.model, "ctx_model",
                        "sentence-transformers/all-MiniLM-L6-v2")
    ctx_dim   = get_ctx_dim(ctx_model)
    doc_ctx     = encode_documents(docs, model_name=ctx_model,
                                   cache_dir=ctx_cache, device=str(device))
    word_embeds = encode_vocabulary(dictionary, model_name=ctx_model,
                                    cache_dir=ctx_cache, device=str(device))
    ...
    _LOG.info(f"{'Variant':<12}{'C_V':>8}{'C_NPMI':>10}{'U_Mass':>10}"
              f"{'C_UCI':>10}{'Div':>8}{'Intra':>8}{'Inter':>8}{'# topics':>10}")
    _LOG.info("=" * 96)
    for r in all_results:
        _LOG.info(f"{r['variant']:<12}{r['C_V']:>8.3f}{r['C_NPMI']:>10.3f}"
                  f"{r['U_Mass']:>10.3f}{r['C_UCI']:>10.3f}"
                  f"{r['Diversity']:>8.3f}{r['Intra']:>8.3f}{r['Inter']:>8.3f}"
                  f"{r['n_topics']:>10d}")
    _LOG.info("=" * 96)
