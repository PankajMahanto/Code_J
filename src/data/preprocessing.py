...
class PreprocessingPipeline:
    ...
    def _build_dictionary(self, docs: List[List[str]]) -> Dictionary:
        ...
        # Aggressive rare/frequent word filtering — fixes very negative
        # U_Mass and removes ubiquitous tokens that pollute coherence stats.
        d.filter_extremes(
        ...
        # Optional second-stage IDF / hapax sweep — removes tokens that
        # appear only once across the *filtered* corpus, which otherwise
        # break C_NPMI for short-text datasets.
        extra_min = int(getattr(self.cfg.preprocessing, "extra_min_count", 0))
        if extra_min > 0:
            doc_freq = Counter()
            id2tok = {i: t for t, i in d.token2id.items()}
            for doc in docs:
                for tok in set(doc):
                    if tok in d.token2id:
                        doc_freq[tok] += 1
            drop_ids = [d.token2id[t] for t, c in doc_freq.items() if c < extra_min]
            if drop_ids:
                d.filter_tokens(bad_ids=drop_ids)
        d.compactify()
