...
# ╚══════════════════════════════════════════════════════════════════════════╝
"""
# Install deps (sentence-transformers replaces the old GloVe path)
!pip install -q gensim==4.3.2 pyyaml sentence-transformers transformers
...
# GloVe is no longer used — sentence-transformers is downloaded automatically.
cfg['dataset']['input_file'] = '/kaggle/input/twitter-data/tweets_dataset.txt'
cfg['dataset']['work_dir']   = f'/kaggle/working/{DATASET}'

# Pick the contextual encoder. all-MiniLM-L6-v2 is fast and 384-dim;
# for Twitter-specific work try cardiffnlp/twitter-roberta-base.
cfg['model']['contextual_model'] = 'sentence-transformers/all-MiniLM-L6-v2'
...
x      = torch.rand(B, V)
x_ctx  = torch.randn(B, d)        # mock contextual sentence embedding
# rebuild model with matching contextual_dim for the smoke test
model = EDNeuFTMv2(vocab_size=V, topic_dim=K,
                   embed_dim=d, hidden_dim=H,
                   word_embeds=dummy_embed,
                   pmi_matrix=dummy_pmi,
