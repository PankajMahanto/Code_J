...
# ─── regex + stopwords ───────────────────────────────────────────────────
# Aggressive stopword list — needed to recover U_Mass into [-1, -0.5].
# A high-frequency junk token bursts the U_Mass count and tanks the score.
_DOMAIN_STOPWORDS = {
    # twitter/web junk
    "rt", "via", "amp", "http", "https", "www", "com", "org", "net",
    "tweet", "retweet", "tweets", "twitter", "facebook", "instagram",
    "youtube", "tiktok", "reddit", "linkedin", "snapchat", "whatsapp",
    # reporting verbs
    "said", "say", "says", "saying", "tell", "told", "tells", "telling",
    "ask", "asked", "asking", "asks",
    # modals + filler
    "would", "could", "should", "may", "might", "must", "shall",
    "also", "even", "still", "yet", "already", "however", "therefore",
    "though", "although", "indeed", "perhaps", "maybe",
    # vague nominals / pronouns
    "one", "two", "three", "first", "second", "third", "last", "next",
    "thing", "things", "way", "ways", "lot", "lots", "bit",
    "anyone", "everyone", "someone", "nobody", "everybody", "anybody",
    "somebody", "anything", "everything", "something",
    # generic verbs of motion / making
    ...
    "very", "pretty", "quite", "kind", "sort", "rather", "fairly",
    "always", "never", "often", "sometimes", "usually", "today",
    "yesterday", "tomorrow", "now", "later", "soon", "ago",
}
...
class PreprocessingPipeline:
    ...
    def _entropy_filter(self, docs: List[List[str]]) -> List[List[str]]:
        """
        Aggressive context-entropy filter: drop the bottom 15 % (rare, narrow
        distribution → noisy) and the top 5 % (ubiquitous, hurts U_Mass /
        C_NPMI).  Configurable via cfg.preprocessing.entropy_low / .high.
        """
        ...
        if not ent:
            return docs
        pcfg  = self.cfg.preprocessing
        low   = float(getattr(pcfg, "entropy_low_pct",  15))
        high  = float(getattr(pcfg, "entropy_high_pct", 95))
        vals  = np.array(list(ent.values()))
        floor = np.percentile(vals, low)
        ceil  = np.percentile(vals, high)
        keep  = {w for w, e in ent.items() if floor <= e <= ceil}

        filt = [[w for w in d if w in keep] for d in docs]
        filt = [d for d in filt if len(d) >= pcfg.min_doc_len]
        _LOG.info(
            f"  entropy band [p{low:g}={floor:.3f}, p{high:g}={ceil:.3f}] "
            f"-> dropped {len(ent)-len(keep):,} words ; "
