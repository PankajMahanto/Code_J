...
# =============================================================================
# Orthogonal regularization — penalize off-diagonal cosine similarity of
# the decoder's topic-word weight matrix.
# =============================================================================
def orthogonal_regularization(weight: torch.Tensor) -> torch.Tensor:
    """
    Force the rows of the decoder weight matrix to be mutually orthogonal.

    Given a [K, V] topic matrix W (e.g. the SCAD decoder β), compute
    the cosine similarity matrix S = W̃ · W̃ᵀ where rows of W̃ are
    L2-normalised. The penalty is the mean SQUARED off-diagonal element
    (i.e., we drive S → I_K).

        loss = ||  S − I_K  ||_F^2 / (K * (K − 1))

    Squaring is important: it makes the loss a smooth, non-negative
    target for both positive and negative similarity, and the gradient
    flows through every off-diagonal entry. This directly attacks the
    Inter-cosine ≤ 0.30 target while pushing Intra-coherence up.
    """
    K = weight.shape[0]
    Wn = F.normalize(weight + 1e-12, dim=-1)
    S  = Wn @ Wn.T                                  # [K, K]
    eye = torch.eye(K, device=weight.device, dtype=weight.dtype)
    off = S - eye                                   # zero on diagonal
