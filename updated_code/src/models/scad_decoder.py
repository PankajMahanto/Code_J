...
============================================================================
 ...
 WHY v2 FAILED
 ─────────────
 Previous version used full Sinkhorn OT with Mahalanobis on raw GloVe.
 With random-init M_U and concept_anchors, the cost matrix was nearly
 uniform → β came out nearly uniform → no gradient signal → rec loss
 stuck at 38.16 for 15 epochs.
...
class SCADecoder(nn.Module):
    ...
    def __init__(self,
        ...
        # (a) Concept anchors — initialized from GloVe centroid (NOT random!)
        # This is critical — random init makes the loss flat.
    ...
    # ------------------------------------------------------------------
    # `weight` view for orthogonal regularization.
    #
    # The orthogonal regularizer in src/training/losses.py expects a
    # K × V "decoder weight matrix" whose rows are topic-word vectors.
    # SCAD does not have a single nn.Linear, so we expose β itself
    # (the effective topic-word distribution) under the conventional
    # `decoder.weight` name — this matches the user's spec
    # "model.decoder.weight" without breaking the novelty internals.
    # ------------------------------------------------------------------
    @property
