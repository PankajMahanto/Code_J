...
# =============================================================================
# Orthogonal Regularization — MANDATORY for Inter-cosine ≤ 0.30
# =============================================================================
def orthogonal_regularization(weight: torch.Tensor) -> torch.Tensor:
    """
    Squared off-diagonal cosine-similarity penalty on a topic-word matrix.

        W_n  = W / ‖W‖₂      (row-normalised)
        S    = W_n · W_n^T   (cosine similarity matrix)
        ℒ    = ‖ S − I ‖²_F / (K · (K − 1))

    Driving ℒ → 0 forces the K topic vectors to be mutually orthogonal.
    This is the direct lever for both the *Inter-cosine ≤ 0.30* and the
    *Intra-coherence ≥ 0.85* targets because orthogonal rows are perfectly
    anti-redundant (low inter) and sharply concentrated (high intra).
    """
    K = weight.shape[0]
    if K < 2:
        return torch.zeros((), device=weight.device, dtype=weight.dtype)
    Wn = F.normalize(weight + 1e-12, dim=-1)           # [K, V]
    S  = Wn @ Wn.T                                      # [K, K]
    I  = torch.eye(K, device=weight.device, dtype=weight.dtype)
    off = S - I
