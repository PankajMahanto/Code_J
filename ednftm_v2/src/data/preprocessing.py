...
_DOMAIN_STOPWORDS = {
    ...
    "tweet", "retweet", "twitter", "facebook", "instagram", "url",
    # reporting verbs — aggressively dropped (hurt U_Mass)
    "said", "say", "says", "saying", "told", "tell", "ask", "asked",
    "think", "thinks", "thought", "know", "known", "knew", "believe",
    "feel", "felt", "seem", "seemed", "find", "found",
    "would", "could", "should", "may", "might", "must", "shall", "will",
    # discourse markers
    "also", "even", "still", "yet", "already", "however", "therefore",
    "though", "although", "thus", "hence", "rather", "quite", "pretty",
    "mean", "really", "actually", "basically", "simply", "literally",
    # quantity / ordinals (low semantic value for topics)
    "one", "two", "three", "four", "five", "six", "seven", "eight",
    "nine", "ten", "first", "second", "third", "last", "next", "half",
    # vague nouns
    "thing", "things", "way", "ways", "lot", "lots", "bit", "kind",
    "part", "side", "case", "cases", "point", "points",
    "time", "times", "day", "days", "week", "weeks", "year", "years",
    "something", "anything", "nothing", "everything",
    "someone", "anyone", "everyone", "people", "person", "guy", "man",
    "woman", "men", "women",
    # support verbs
    "get", "gets", "got", "getting", "go", "goes", "going", "gone",
    "make", "makes", "made", "making",
    "take", "takes", "took", "taken", "taking",
    "come", "comes", "came", "coming",
    "see", "saw", "seen", "seeing", "look", "looked", "looking",
    "use", "used", "using", "work", "worked", "working",
    "try", "tried", "trying", "start", "started", "starting",
    "put", "puts", "putting", "give", "gave", "given", "giving",
    "let", "keep", "kept", "want", "wanted", "wants", "need", "needed",
    # intensifiers / fillers
    "like", "well", "back", "much", "many", "few", "several",
    "always", "never", "often", "sometimes", "maybe", "probably",
    "stuff", "hey", "wait", "remember", "wonder", "fun", "happen",
    "happens", "happened", "somebody", "anybody", "guess", "glimpse",
}
...
class PreprocessingPipeline:
    ...
    def _build_dictionary(self, docs: List[List[str]]) -> Dictionary:
        ...
        # ── first pass: aggressive df-based filter ──────────────────────
        d.filter_extremes(
            no_below=pcfg.min_word_count,
            no_above=pcfg.max_word_freq,
            keep_n=pcfg.max_vocab,
        )
        d.compactify()
        _LOG.info(f"  dict after df filter: {len(d):,} "
                  f"(no_below={pcfg.min_word_count}, "
                  f"no_above={pcfg.max_word_freq}, "
                  f"keep_n={pcfg.max_vocab})")

        # ── second pass: tail & head pruning ─────────────────────────────
        # Drop the 1 % rarest AND the 1 % most-frequent tokens in the
        # filtered vocabulary — these still dominate PMI noise and U_Mass.
        tail_pct = float(getattr(pcfg, "rare_tail_pct", 0.01))
        head_pct = float(getattr(pcfg, "common_head_pct", 0.01))
        if tail_pct > 0.0 or head_pct > 0.0:
            dfs = np.array([d.dfs[i] for i in range(len(d))])
            n = len(dfs)
            if n > 10:
                tail_cut = int(np.quantile(dfs, tail_pct))
                head_cut = int(np.quantile(dfs, 1.0 - head_pct))
                bad_ids = [i for i in range(len(d))
                           if d.dfs[i] <= tail_cut or d.dfs[i] >= head_cut]
                if bad_ids:
                    d.filter_tokens(bad_ids=bad_ids)
                    d.compactify()
                    _LOG.info(f"  dict after tail/head prune: {len(d):,} "
                              f"(dropped tail≤{tail_cut}, head≥{head_cut})")
        return d
